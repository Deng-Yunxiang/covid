...
"""Compute the accuracy over the testset"""
def bagging(model_num):
    model_list = []
    model_list = generatebaggingModels(model_num)
    # All predictions generated by the models are stored in a dictionary
    pred_dic = {}

    # Each model in the list is looped to generate prediction
    ...
    # Sum up all the values generated by all the models
    total = sum(value)
    # Generate the final prediction  
    pred_list = torch.round((total/model_num))
    # Compute the accuracy by comparing the prediction with the actual labels
